    ...
    for (let i = 0; i < packages.length; i++) {
      ...
      await db.tradeRecommendation.create({
        data: {
          ...
          week,
          season,
          myPlayers: pkg.myPlayers,
          targetPlayers: pkg.targetPlayers,
          ...
          sellHighPlayers: pkg.myPlayers.filter((p) => p.isSellHigh),
          buyLowPlayers: pkg.targetPlayers.filter((p) => p.isBuyLow),
